...
function jarvis_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		jarvis_custom_options_show_loader();
		//location.reload();
		var loc = jQuery('#co_site_url').val();
		var params = JARVIS_STORAGE['co_add_params']!=undefined ? JARVIS_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#jarvis-customizer-styles-'+option).length > 0 ? jQuery('#jarvis-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="jarvis-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
